        ...
        <div class="p-4 md:p-16 flex flex-col  w-full">
            <div class="flex w-full flex-col 2xl:flex-row gap-4 ">
                <div class="flex flex-col w-full">
                    ...
                    <h1 class="font-Alumni font-semibold text-md md:text-lg mt-2">Parler nous des services que vous offrez
                    </h1>
                ...
                <div>
                    ...
                    <div class="bg-primary-100 py-8 px-4 mt-8">
                        ...
                        <div class="mt-6 w-full max-w-md flex gap-4 columns-2">
                            ...
                            <div class="w-1/2">
                                <label for="numeroLicence" class="block font-Alumni text-md md:text-lg mb-2">Numero de
                                    licence</label>
                            ...
                            <div class="w-1/2">
                                ...
                                <select name="statut" id="statut"
                                    ...
                                    <option value="non valide"
                                        {{ session('licences.statut') == 'Non valide' ? 'selected' : '' }}>Non valide
                                    </option>
                                    <option value="expirée" {{ session('licences.statut') == 'Expirée' ? 'selected' : '' }}>
                                        Expirée</option>
                                    <option value="suspendue"
                                        {{ session('licences.statut') == 'Suspendue' ? 'selected' : '' }}>Suspendue</option>
                                    <option value="révoquée"
                                        {{ session('licences.statut') == 'Révoquée' ? 'selected' : '' }}>Révoquée</option>
                                    <option value="non renouvelée"
                                        {{ session('licences.statut') == 'Non renouvelée' ? 'selected' : '' }}>Non
                                        renouvelée</option>
                                    <option value="annulée"
                                        {{ session('licences.statut') == 'Annulée' ? 'selected' : '' }}>Annulée</option>
                                </select>
                                @error('statut')
                                    <span
                                        class="font-Alumni text-lg flex items-center font-medium tracking-wide text-red-500 text-xs mt-1 ml-1">
                        ...
                        <div class="mt-6 w-full max-w-md flex gap-4 columns-2">
                            <div class="w-full">
                                <label for="typeLicence" class="block font-Alumni text-md md:text-lg mb-2">Type de
                                    licence</label>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function renderChecklist(data) {
                ...
                if (!data || Object.keys(data).length === 0) {
                    checklistContainer.innerHTML =
                        '<p>Aucune sous-catégorie disponible pour ce type de licence.</p>';
                    return;
                }
